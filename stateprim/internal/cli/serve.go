...
import (
	...
	"github.com/propifly/primkit/stateprim/internal/api"
...
func newServeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Starts stateprim in HTTP server mode. The API is served under /v1/ and
...
WAL frames to the configured object storage (S3, R2, B2).
  ...
  stateprim serve --port 8091 --config /etc/stateprim/config.yaml`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			logger.Info("stateprim serve starting", "port", port)
			return srv.ListenAndServe(ctx)
		},
	}

	cmd.Flags().IntVarP(&port, "port", "p", 8091, "HTTP port to listen on")
