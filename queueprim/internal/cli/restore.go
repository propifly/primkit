...
The replica configuration (provider, bucket, path, credentials) is read from
the config file or environment variables. The restored database is written to
the configured database path.

  queueprim restore --config /etc/queueprim/config.yaml`,
